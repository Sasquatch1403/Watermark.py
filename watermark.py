...
POST_CONFIRM_DELAY_MIN = 1
POST_CONFIRM_DELAY_MAX = 2
...
def get_pole_id(filename):
    """
    Extract pole ID = digits before first underscore.
    Example: '603504_2025-11-25_xx.jpg' -> '603504'
    """
...
def watermark_image(path_in, path_out):
    ...
    Returns True if written, False if skipped (e.g. no numeric pole ID).
    """
    pole_id = get_pole_id(path_in.name)
    if pole_id is None:
        print(f"    Skipping {path_in.name}: no numeric pole ID found.")
        return False
...
def main():
    ...
    # Final summary (typewriter + different top/bottom lengths)
    summary1 = f"Found {num_stills_dirs} 'stills' folders with images."
    ...
    # Top line matches the length of the first summary
    top_len = len(summary1)
    # Bottom line matches the length of the last summary
